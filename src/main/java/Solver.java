...
public class Solver {
    ...
    private static Comparator<Node> hammingPriority = new Comparator<Node>() {
        @Override
        public int compare(Node o1, Node o2) {
            return (o1.board.hamming() + o1.moves) -
                    (o2.board.hamming() + o2.moves);
        }
    };
    ...
    public Solver(Board initial) {
        ...
        queue = new MinPQ<>(manhattanPriority);
        twinQueue = new MinPQ<>(manhattanPriority);
        ...
        while (!solvable && !twinSolvable) {
            ...
            for (Board neighbor : searchNode.board.neighbors()) {
                if (searchNode.previousNode != null &&
                        neighbor.equals(searchNode.previousNode.board)) {
                    continue;
                }
            ...
            for (Board twinNeighbor : twinSearchNode.board.neighbors()) {
                if (twinSearchNode.previousNode != null &&
                        twinNeighbor.equals(twinSearchNode.previousNode.board)) {
                    continue;
                }
                twinQueue.insert(new Node(twinNeighbor,
